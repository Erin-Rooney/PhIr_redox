...
thaw_depths_fig_violin_2021_2022_nonacidic =
  ...
  filter(Area == "non-acidic tundra") %>% 
  ggplot()+
  geom_violin(aes(x = as.Date(date2), y = thaw_depth_cm, group = as.Date(date2), fill = Site))+
...
# thaw_depths_fig_violin_2022_nonacidic_hydric =
#   thawdepths2021_2022 %>%
#   mutate(Site = factor(Site, levels = c("Dry", "Mesic", "Hydric"))) %>% 
#   filter(Area == "non-acidic tundra" & Site == "Hydric" & Year == 2022) %>% 
#   ggplot()+
#   geom_violin(aes(x = as.Date(date2), y = thaw_depth_cm, group = as.Date(date2), fill = Site))+
#   labs(title = " ",
#        x = " ",
...
ggsave("formanuscript/thaw_depths_fig_violin_2022_nonacidic_hydric.png", plot = thaw_depths_fig_violin_2022_nonacidic_hydric, height = 4, width = 4)

thaw_depths_fig_point_2021_nonacidic_hydric =
  thawdepths2021_2022 %>%
  mutate(Site = factor(Site, levels = c("Dry", "Mesic", "Hydric"))) %>%
  mutate(label = paste0(Month_num, "-", Day)) %>% 
  filter(Area == "non-acidic tundra" & Site == "Hydric" & Year == 2021) %>% 
  ggplot()+
  geom_jitter(aes(x = as.Date(date2), y = thaw_depth_cm), fill = "#b5838d", color = "#6d6875", width = 4, size = 1, shape = c(21))+
  geom_smooth(aes(x = as.Date(date2), y = thaw_depth_cm), color = "#6d6875", method = "loess")+
  labs(title = " ",
       x = " ",
       y = "thaw depth (cm)")+
  #geom_rect(aes(xmin=as_date('2021-06-15'), xmax= as_date('2021-08-09'), ymin=49.5, ymax=50.5), fill = "black")+
  ...
  facet_grid(Site~., scales = "free")+
  theme_er1()+
  theme(legend.position = "right", axis.text.x = element_text(vjust = 0.5, hjust=0.5, angle = 45, size = 13),
...
thaw_depths_fig_point_2021_allsites =
  ...
  filter(Year == 2021) %>% 
  ggplot()+
  geom_jitter(aes(x = as.Date(date2), y = thaw_depth_cm, fill = Site, color = Site), width = 4, size = 1.75, alpha = 0.4, shape = c(21))+
...
ggsave("formanuscript/thaw_depths_fig_smoothpointsallsites.png", plot = thaw_depths_fig_point_2021_allsites, height = 7, width = 7)
...
thaw_depths_fig_violin_nonacidic_dry =
  ...
  ggplot()+
  geom_rect(aes(xmin =as_date('2021-06-15'), xmax= as_date('2021-09-06'), ymin=4.75, ymax=5.25), color = c("#9b2226"), fill = c("#9b2226"))+
  geom_rect(aes(xmin =as_date('2021-06-15'), xmax= as_date('2021-09-06'), ymin=14.75, ymax=15.25), color = c("#EDA24E"), fill = c("#EDA24E"))+
  geom_rect(aes(xmin =as_date('2021-06-15'), xmax= as_date('2021-09-06'), ymin=24.75, ymax=25.25), color = c("#ffd60a"), fill = c("#ffd60a"))+
  geom_violin(aes(x = as.Date(date2), y = thaw_depth_cm, group = as.Date(date2)), fill = "black", alpha = 0.4)+
  labs(x = "",
       y = "Thaw Depth, cm")+
  #scale_fill_gradientn(colors = natparks.pals(name = "Banff"))+
  scale_x_date(date_breaks = "1 week", date_labels = "%b-%d")+
...
thaw_depths_fig_violin_nonacidic_hydric =
  ...
  geom_rect(aes(xmin =as_date('2021-06-15'), xmax= as_date('2021-09-06'), ymin=14.75, ymax=15.25), color = c("#EDA24E"), fill = c("#EDA24E"))+
  geom_rect(aes(xmin =as_date('2021-06-15'), xmax= as_date('2021-09-06'), ymin=24.75, ymax=25.25), color = c("#ffd60a"), fill = c("#ffd60a"))+
  geom_rect(aes(xmin =as_date('2021-06-15'), xmax= as_date('2021-09-06'), ymin=54.75, ymax=55.25), color = c("#31572c"), fill = c("#31572c"))+
  geom_violin(aes(x = as.Date(date2), y = thaw_depth_cm, group = as.Date(date2)), fill = "black", alpha = 0.4)+
  labs(x = "",
       y = "Thaw Depth, cm")+
 # geom_rect(aes(xmin=as_date('2021-06-15'), xmax= as_date('2021-08-09'), ymin=49.5, ymax=50.5), fill = "black")+
