...
fig_2021 =
  ggplot()+
  geom_point(data = df2, aes(y = depth_cm, x = as_datetime(Betterdate)), color = "#a99985", fill = "#a99985", size = 1.5, alpha = 0.4, shape = c(21))+
  geom_point(data = df1, aes(y = depth_cm, x = as_datetime(Betterdate)), color = "#1be7ff", fill = "#1be7ff", size = 1.5, shape = c(21))+
  labs(y = "depth, cm",
       x = "2021")+
  ...
  facet_grid(position~site)+
  theme_er1()+
  theme(legend.position = "bottom", axis.text.x = element_text (size = 10 , vjust = 0.5, angle = 45))

ggsave("formanuscript/frozen_2021_fig.tiff", plot = fig_2021, height = 6.5, width = 7.5)


fig_2021_b =
  ggplot()+
  geom_point(data = df2, aes(y = avg_values_fixed, x = as_datetime(Betterdate), color = "#a99985", fill = "#a99985"), size = 2, alpha = 0.4, shape = c(21))+
...
fig_2022 =
  ggplot()+
  #geom_point(aes(y = depth_cm, x = as_datetime(Betterdate), fill = frozen, color = frozen), size = 1.5, alpha = 0.4, shape = c(21))+
  geom_point(data = df4, aes(y = depth_cm, x = as_datetime(Betterdate)), color = "#a99985", fill = "#a99985", size = 1.5, alpha = 0.4, shape = c(21))+
  geom_point(data = df3, aes(y = depth_cm, x = as_datetime(Betterdate)), color = "#1be7ff", fill = "#1be7ff", size = 1.5, shape = c(21))+
    labs(y = "depth, cm",
       x = "2022")+
  scale_x_datetime(date_breaks = "1 week", date_labels = "%b-%d")+
  # scale_color_manual(values = c("#1be7ff", "#dad2bc"))+
  # scale_fill_manual(values = c("#1be7ff", "#dad2bc"))+
  scale_y_reverse()+
  #scale_x_continuous(position="top")+
  facet_grid(position~site)+
  theme_er1()+
...
ggsave("formanuscript/frozen_2022_fig.tiff", plot = fig_2022, height = 6.5, width = 7.5)
